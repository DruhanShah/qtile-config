...
class _GroupBase(base._TextBox, base.PaddingMixin, base.MarginMixin):
    ...
    def box_width(self, groups):
        width, _ = self.drawer.max_layout_size(
            [self.fmt.format(i.label)
             for i in groups], self.font, self.fontsize, self.markup
        )
        return width + self.padding_x * 2 + self.borderwidth * 2

    def box_height(self, groups):
        height, _ = self.drawer.max_layout_size(
            [self.fmt.format(i.label)
             for i in groups], self.font, self.fontsize
        )
    ...
    def _configure(self, qtile, bar):
        ...
        self.layout = self.drawer.textlayout(
            "", "ffffff",
            self.font, self.fontsize, self.fontshadow, markup=self.markup
        )
        self.setup_hooks()
    ...
    def drawbox(
        ...
        framed = self.layout.framed(
            border_width, framecolor, 0, pad_y, highlight_color)
        y = self.margin_y
        x = self.margin_x
        if self.center_aligned:
            for t in base.MarginMixin.defaults:
                if t[0] == "margin":
                    ...
                    x += (self.bar.width - framed.width) / 2 - t[1]
                    break
        ...
        if block and bordercolor is not None:
            framed.draw_fill(x, y, rounded)
        elif line:
            framed.draw_line(x, y, highlighted)
        else:
            framed.draw(x, y, rounded)
...
class V_GroupBox(_GroupBase):
    ...
    def draw(self):
        ...
        self.drawer.draw(offsetx=self.offsetx,
                         offsety=self.offset, width=self.width)
